...
class ForoViewModel : ViewModelForoBase() {
    ...
    fun obtenerComentarios() {
        viewModelScope.launch {
            try {
                val response = RetrofitClient.apiComentarios.obtenerComentarios()
    ...
    fun comprimirImagenA100KB(context: Context, uri: Uri): File {
        val inputStream = context.contentResolver.openInputStream(uri)
            ?: throw Exception("No se pudo abrir imagen")
    ...
    fun crearComentario(context: Context) {
        ...
        viewModelScope.launch {
            try {
                ...
                val comentarioRequestBody =
                    jsonComentario.toRequestBody("application/json; charset=utf-8".toMediaType())
                ...
                val response =
                    RetrofitClient.apiComentarios.crearComentario(comentarioRequestBody, imagenPart)
